...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def loop(chars: List[Char], acc: List[(Char,Int)]): List[(Char, Int)] = {
        ...
        else loop(chars.filter(p => p != chars.head), (chars.head, chars.count(p => p == chars.head)) :: acc)
      }
      loop(chars, Nil)
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      def loop(freqs: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = {
        ...
        else loop(freqs.tail, Leaf(freqs.head._1, freqs.head._2) :: acc)
      }
      loop(freqs.sortWith(_._2 > _._2), Nil)
    }
   ...
   */
    def until(f_stop: List[CodeTree] => Boolean, f_loop: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (f_stop(trees)) trees
      else until(f_stop, f_loop)(f_loop(trees))
    }
   ...
   */
    def createCodeTree(chars: List[Char]): CodeTree = {
      val freqs_list = makeOrderedLeafList(times(chars))
      until(singleton, combine)(freqs_list).head
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def loop(tree:  CodeTree)(c: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) if chars(left).contains(c) => 0 :: loop(left)(c)
        case Fork(left, right, _, _) => 1 :: loop(right)(c)
      }
