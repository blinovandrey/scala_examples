...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.groupBy(_.toLower).toList.map(x => (x._1, x._2.length)).sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      } yield (occurrences.head._1, n) :: combo) ++ combos
    }
  }
